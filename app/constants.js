// app/constants.js — данные + модели нормализации DRIVEX
(() => {
  'use strict';
  window.DX = window.DX || {};
  const DX = window.DX;
  // Экспортируем garageCars/savedPlaces как mutable refs
  let garageCars = [];
  let savedPlaces = [];
  DX._garageCarsRef  = { get val() { return garageCars; }, set val(v) { garageCars=v; } };
  DX._savedPlacesRef = { get val() { return savedPlaces; }, set val(v) { savedPlaces=v; } };
  const quickActions = [
    {
      icon: "wrench",
      label: "Ремонт",
  ...
  // Export key items to DX
  // Export functions to DX
  // Export consts to DX
})();
